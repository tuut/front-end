...
function get_search_form( $echo = true ) {
	...
	if ( '' != $search_form_template ) {
		...
		if ( 'html5' == $format ) {
			$form = '<form role="search" method="get" class="search-form search-home" action="' . esc_url( home_url( '/' ) ) . '">
				<label>
					<input type="search" class="search-field" placeholder="' . esc_attr_x( 'Digite sua busca', 'placeholder' ) . '" value="' . get_search_query() . '" name="s" />
				</label>
				<input type="submit" class="search-submit icon-lupa" value="'. esc_attr_x( 'Buscar', 'submit button' ) .'" />
			</form>';
		} else {
